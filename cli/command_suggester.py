...
class CommandSuggester:
    ...
    @staticmethod
    def suggest_command(user_input: str, available_commands: List[str] = None) -> List[Tuple[str, float]]:
        """
        ...
        for cmd in available_commands:
            ...
            similarity = difflib.SequenceMatcher(None, user_input_lower, cmd.lower()).ratio()
            suggestions.append((cmd, similarity))
        
        # Sort by similarity (highest first) and filter out very low matches
        suggestions.sort(key=lambda x: x[1], reverse=True)
        suggestions = [s for s in suggestions if s[1] > 0.3]  # Only suggest if >30% similar
